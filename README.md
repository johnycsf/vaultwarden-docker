...
One-command Vaultwarden for homelab beginners — official `vaultwarden/server`, backup-before-update.
...
`./manage.sh` opens a **↑/↓ menu** with a `>` cursor (j/k and Enter also work). Open the URL printed by the script, create your account, then disable signups as instructed.
...
Incremental snapshots via `rsync` hardlinks (unchanged files are not re-copied). `./manage.sh update` uses this same `backup.sh` before updating (into `./backups`). Keep the backup root on **one filesystem** so hardlinks work.
...
# On this machine or a brand-new host after ./manage.sh:
...
**Database safety:** Vaultwarden is stopped or scaled to 0, WAL-checkpointed when `sqlite3` is available, integrity-checked, then copied. Incremental hardlinks apply to file trees; each SQL dump is a full verified file with a SHA-256 in `META.txt`.
